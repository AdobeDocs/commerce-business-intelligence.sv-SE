...
The `sales_order` tabell (`sales_flat_order` på M1) är där varje order hämtas. I de flesta fall representerar varje rad en unik order, även om det finns några anpassade implementeringar av Commerce som leder till att en order delas upp i separata rader.

Det här registret innehåller alla kundorder, oavsett om beställningen har bearbetats via gästutcheckning eller inte. Om din butik accepterar utcheckning av gäster kan du få mer information om detta [användningsfall](../data-warehouse-mgr/guest-orders.md).
...
| `coupon_code` | Kupongen tillämpas på ordern. Om ingen kupong används kommer detta fält att `NULL` |
| `created_at` | Tidsstämpel för att skapa ordern, som vanligtvis lagras lokalt i UTC. Beroende på din konfiguration i [!DNL MBI]kan den här tidsstämpeln konverteras till en rapporttidszon i [!DNL MBI] som skiljer sig från databasens tidszon |
...
| `Revenue` | Summan av intäkten för alla order, där intäkten definieras som det slutpris som kunden betalar, efter att alla skatter, rabatter, frakt osv. har tillämpats | `Operation: Sum`<br/>`Operand: base_grand_total`<br/>`Timestamp:` `created_at` |
...
* Gå med i `customer_group` för att skapa nya kolumner som returnerar kundgruppnamnet för den kund som lade ordern.
   * Sökväg: `sales_order.customer_group_id` (många) => `customer_group.customer_group_id` (ett)

`sales_order_address`

* Gå med i `sales_order_address` för att skapa nya kolumner som returnerar fakturerings- och leveransplatser som är kopplade till ordern. Det går att ansluta två sökvägar beroende på om fakturerings- eller leveransinformation krävs.
   * Banor:
...
* Gå med i `store` för att skapa nya kolumner som returnerar information som är relaterad till det Commerce Store som är associerat med ordern.
